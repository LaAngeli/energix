...
|   - separatorul general pune tabloul sub tensiune (si tu il poti comuta);
|   - diferentialul de 30 mA are buton TEST care chiar declanseaza si
|     reanclanseaza, exact ca pe un tablou real;
|   - fiecare disjunctor se comuta si isi aprinde / stinge consumatorul;
|   - voltmetrul numara pana la 230 V (constanta fizica, nu cifra de marketing).
|
| La prima intrare in cadru tabloul se energizeaza singur, o data. Dupa aceea
| nu se mai misca nimic fara actiunea utilizatorului.
|
| Accesibilitate: comutatoarele sunt <button role="switch">, starea se anunta
| printr-un status sr-only, iar prefers-reduced-motion primeste starea finala
...
<div
    ...
    data-live="false"
>
    {{-- eticheta de tablou, ca pe usa unui tablou real --}}
    <p class="flex items-center justify-between gap-3 border-b border-line pb-3 font-mono text-[0.6rem] tracking-[0.18em] text-paper-dim uppercase">
        <span>Tablou distribuție · TD-01</span>
        <span>230 V ~ 50 Hz</span>
    </p>

    {{-- ------------------------------------------------ randul de alimentare --}}
    <div class="mt-4 flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-mono text-[0.6rem] tracking-[0.18em] text-paper-dim uppercase">Rețea</p>
        ...
        >
            <span class="text-left">
                <span class="block font-mono text-[0.6rem] tracking-[0.18em] text-paper-dim uppercase">Separator</span>
                <span class="block font-mono text-[0.6rem] tracking-[0.18em] text-gold uppercase" data-master-state>Oprit</span>
            </span>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-mono text-[0.6rem] tracking-[0.18em] text-cyan uppercase">Diferențial 30 mA</p>
                <button
                    type="button"
                    data-rcd-test
    ...
    {{-- ---------------------------------------------------------- circuitele --}}
    <ol class="grid grid-cols-3 gap-x-2 lg:grid-cols-6" role="group" aria-label="Circuitele tabloului — comută disjunctoarele">
        @foreach (config('energix.panel_circuits') as $circuit)
            <li class="circuit flex flex-col items-center" data-circuit>
                ...
                <button
                    ...
                    role="switch"
                    aria-checked="true"
                    aria-label="Circuit {{ $circuit['name'] }}, {{ $circuit['amps'] }}"
                    class="breaker group"
                >
                ...
                <span class="circuit-load">
                    ...
                    <span class="font-mono text-[0.58rem] tracking-[0.12em] uppercase">{{ $circuit['name'] }}</span>
                </span>
            </li>
        @endforeach
    </ol>
    ...
    <p class="mt-4 border-t border-line pt-3 text-center font-mono text-[0.6rem] tracking-[0.14em] text-paper-dim uppercase">
        Comută disjunctoarele — tabloul e funcțional
    </p>
